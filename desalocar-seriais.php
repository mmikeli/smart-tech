<?php
  ...
  $pathPage = '
    ...
    <h6 class="font-weight-bolder mb-0">Desalocar Seriais</h6>
  ';
...
<body class="g-sidenav-show  bg-gray-100">
        ...
        <div class="col-12">
          <div class="card mb-4">
            <div class="card-header pb-0">
              <h6>Desalocar Seriais</h6>
            </div>
            <div class="card-body">
              <form class="row" role="form" method="POST" action="sys/update.php?desalocar">

                <div class="col-lg-5 col-md-5 col-sm-12 col-12">
                  <label>De Quem?</label>
                    <select id="inputRevenda" name="revendedor" class="form-control select2" required>
                ...
                <div class="col-lg-5 col-md-5 col-sm-12 col-12">
                  <label>Produto</label>
                    <select id="inputProduto" name="produto" class="form-control select2" required>
                    	<option selected disabled>:: SELECIONE ::</option>
                      <?php while($dadosP = $qryP->fetch_assoc()){ ?>
                      <option value="<?php echo $dadosP['chave']; ?>"><?php echo $dadosP['nome']. ' ' .$dadosP['categoria']; ?></option>
                      <?php } ?>
                    </select>
                    <a class="font-weight-bold">Quantidade Disponível: <span id="qtdDisponivel">0</span></a>
                </div>
  ...
  <script type="text/javascript">
  	...
  	$(document).ready(function(){
        ...
        $("#qtdDisponivel").html("<i class='fas fa-spinner'></i>");

	  		let rev = $("#inputRevenda option").filter(':selected').val();
        let prod = $("#inputProduto option").filter(':selected').val();

        $.ajax({

          url: 'sys/catch.php?desalocar&r='+rev+'&p='+prod,
          type: 'POST',
          success: function(retorno){

            $("#qtdDisponivel").html(retorno);
